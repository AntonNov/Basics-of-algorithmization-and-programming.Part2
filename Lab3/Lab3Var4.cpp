...
//---------------------------------------------------------------------------
//перекручатель ввода в edit, в зависимости от выбранной опции
...
// действия в зависимости от выбранной функции
void __fastcall TForm1::OKClick(TObject *Sender)
{
	 ...
	 if(ComboBox1->ItemIndex==0)
	{
		try
		{
			...
			for(Node* q=list->head; q; q=q->next)
			{   //проверка на аналогичность
				if(q->getDay()==tmp->getDay() && q->getDestination()==tmp->getDestination()
				   && q->getHH()==tmp->getHH() && q->getMM()==tmp->getMM())
				{   //выбрасывание исключения с кодом ошибки
					throw MyException(0);
				}
			}
			list->Push(tmp);
		...
		{
			ListBox1->Items->Add("ErrorCode:"+IntToStr(ex.ErrorCode())+"ErrorMessage:"+ex.ErrorMessage());
		}catch(...)
		{
