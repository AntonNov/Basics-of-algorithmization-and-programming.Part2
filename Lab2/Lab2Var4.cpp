...
using namespace std;
TApp *App;
//Вектор кандидатов -- является более удобным вариантом динамического массива
...
__fastcall TApp::TApp(TComponent *Owner)
	...
	//LOADING CANDIDATES
	loadFromFile("C:\\Users\\DELL\\Desktop\\Labs\\OAIP 2 sem\\Lab2\\input.txt");
	showMemos(groomsMemo, bridesMemo);
}
//распределение кандидатов в разные memos
void showMemos(TMemo *groomsMemo, TMemo *bridesMemo) {
	bridesMemo->Clear();
	groomsMemo->Clear();
	for (int i = 0; i < candidatesVector.size(); i++) {
		if(candidatesVector[i].getIsFree())
		{
...
//---------------------------------------------------------------------------
 //возможность изменить содержимое по клавише Enter
void __fastcall TApp::idEditKeyPress(TObject *Sender, System::WideChar &Key) {
	if (Key == VK_RETURN && optionComboBox->ItemIndex == 1 && candidatesVector.size() > StrToInt(idEdit->Text) &&
		candidatesVector[StrToInt(idEdit->Text)].getIsFree()) {
...
//---------------------------------------------------------------------------
//сохранить в файл построчно
void __fastcall TApp::SaveButtonClick(TObject *Sender) {
	if (FileExists("C:\\Users\\DELL\\Desktop\\Labs\\OAIP 2 sem\\Lab2\\input.txt")) {
		DeleteFile("C:\\Users\\DELL\\Desktop\\Labs\\OAIP 2 sem\\Lab2\\input.txt");
	}
	TFileStream *fp = new TFileStream("C:\\Users\\DELL\\Desktop\\Labs\\OAIP 2 sem\\Lab2\\input.txt", fmCreate);
	for (int i = 0; i < candidatesVector.size(); i++) {
...
{
	return i.getAge()<=j.getMaxAge() && i.getAge()>=j.getMinAge() && i.getHeight()<=j.getMaxHeight() && i.getHeight()>=j.getMinHeight() &&
...
void findPairs(TMemo *groomsMemo,TMemo *bridesMemo)
{
	bridesMemo->Clear();
	groomsMemo->Clear();
	//сверяем каждый со всеми, не включая выбранный
...
void __fastcall TApp::OpenButtonClick(TObject *Sender)
{
   ...
   //Фильтр
	...
	OpenDialog->InitialDir = "C:\\Users\\DELL\\Desktop\\Labs\\OAIP 2 sem\\Lab2\\input.txt";
